...
---
<p align="center">
  <img src="../../figures/page_0093_fig_1.jpg" alt="Figure 6.10 Line search using the bracketing approach. a) The current solution is at position $a$ (orange point), and we wish to search the region $[a,d]$ (gray" />
</p>

<p align="center">
  <strong>Figure 6.10</strong> Line search using the bracketing approach. a) The current solution is at position $a$ (orange point), and we wish to search the region $[a,d]$ (gray shaded area). We define two points $b,c$ in order to the search region and evaluate the loss function at these points. Here $L[b]>L[c]$, so we eliminate the range $[a,b]$. b) We now repeat this procedure in the refined search region and find that $L[b]<L[c]$, so we eliminate the range $[c,d]$. c) We repeat this process until this minimum is closely bracketed.
</p>

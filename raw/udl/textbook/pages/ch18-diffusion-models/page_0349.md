...
A diffusion model consists of an encoder and a decoder. The encoder takes a data sample x and maps it through a series of intermediate latent variables  $z_{1}\ldots z_{T}$ . The decoder reverses this process; it starts with  $z_{T}$  and maps back through  $z_{T-1},\ldots,z_{1}$  until it finally (re-)creates a data point x. In both encoder and decoder, the mappings are stochastic rather than deterministic.

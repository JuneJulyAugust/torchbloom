...
$$
D_{KL}\left[Norm_{\mathbf{w}}[\mathbf{a},\mathbf{A}]||Norm_{\mathbf{w}}[\mathbf{b},\mathbf{B}]\right]=\frac{1}{2}\left(\mathrm{tr}\left[\mathbf{B}^{-1}\mathbf{A}\right]-d+(\mathbf{a}-\mathbf{b})^{T}\mathbf{B}^{-1}(\mathbf{a}-\mathbf{b})+\log\left[\frac{|\mathbf{B}|}{|\mathbf{A}|}\right]\right). \quad (18.48)
$$

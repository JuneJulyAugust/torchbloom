...
$$
\begin{aligned}Norm_{x}[a,A]& Norm_{x}[b,B]\propto Norm_{x}\Big[(A^{-1}+B^{-1})^{-1}(A^{-1}a+B^{-1}b),(A^{-1}+B^{-1})^{-1}\Big].\end{aligned} \quad (18.47)
$$
...
$$
D_{KL}\left[Norm_{\mathbf{w}}[\mathbf{a},\mathbf{A}]||Norm_{\mathbf{w}}[\mathbf{b},\mathbf{B}]\right]=\frac{1}{2}\left(\mathrm{t r}\left[\mathbf{B}^{-1}\mathbf{A}\right]-d+(\mathbf{a}-\mathbf{b})^{T}\mathbf{B}^{-1}(\mathbf{a}-\mathbf{b})+\log\left[\frac{|\mathbf{B}|}{|\mathbf{A}|}\right]\right). \quad (18.48)
$$
...
$$
\sqrt{\frac{\alpha_{t}}{\alpha_{t-1}}}=\sqrt{1-\beta_{t}}. \quad (18.49)
$$

 Problem 18.10 $^{*}$ If $\alpha_{t}=\prod_{s=1}^{t}\mathbf{1}-\beta_{s}$, then show that: 

$$
\frac{(1-\alpha_{t-1})(1-\beta_{t})+\beta_{t}}{(1-\alpha_{t})\sqrt{1-\beta_{t}}}=\frac{1}{\sqrt{1-\beta_{t}}}. \quad (18.50)
$$

...
$$
h_i \leftarrow \frac{h_i-m_h}{s_h+\epsilon}\qquad \forall i\in\mathcal{B}\qquad (11.8)
$$

where $\epsilon$ is a small number that prevents division by zero if $h\_{i}$ is the same for every member of the batch and $s\_{h}=0$.

Finally, the normalized variable is scaled by $\gamma$ and shifted by $\delta$:

$$
h_i \leftarrow \gamma h_i+\delta\qquad \forall i\in\mathcal{B}\qquad (11.9)
$$

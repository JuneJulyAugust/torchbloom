...
**Number of linear regions:** Consider a shallow network with $D\_i\geq2$-dimensional inputs and $D$ hidden units. The number of linear regions is determined by the intersections of the $D$ hyperplanes created by the "joints" in the ReLU functions (e.g., figure 3.8d-f). Each region is created by a different combination of the ReLU functions clipping or not clipping the input. The number of regions created by $D$ hyperplanes in the $D\_i\leq D$-dimensional input space was shown by Zaslavsky (1975) to be at most $\sum\_{j=0}^{D\_i}\binom{D}{j}$ (i.e., a sum of binomial coefficients). As a rule of thumb, shallow neural networks almost always have a larger number $D$ of hidden units than input dimensions $D\_i$ and create between $2^{D\_i}$ and $2^D$ linear regions.

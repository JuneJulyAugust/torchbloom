...
Each linear region in figure 3.3j corresponds to a different activation pattern in the hidden units. When a unit is clipped, we refer to it as inactive, and when it is not clipped, we refer to it as active. For example, the shaded region receives contributions from $h_1$ and $h_3$ (which are active) but not from $h_2$ (which is inactive). The slope of each linear region is determined by the original slopes $\theta_{\bullet1}$ of the active inputs for this region and the weights $\phi_{\bullet}$ that were subsequently applied. For example, the slope in the shaded region (see problem 3.3) is $\theta_{11}\phi_1+\theta_{31}\phi_3$, where the first term is the slope in panel (g) and the second term is the slope in panel (i).

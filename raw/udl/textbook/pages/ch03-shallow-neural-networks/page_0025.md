...
We can break down this calculation into three parts: first, we compute three linear functions of the input data $(\theta_{10}+\theta_{11}x,\theta_{20}+\theta_{21}x,\text{ and }\theta_{30}+\theta_{31}x)$. Second, we pass the three results through an activation function $a[\bullet]$. Finally, we weight these resulting activations with $\phi_1$, $\phi_2$, and $\phi_3$, sum them, and add an offset $\phi_0$.

...
---
<p align="center">
  <img src="../../figures/page_0277_fig_1.jpg" alt="GAN mechanism. a) Given a parameterized function (a generator) that synthesizes samples (orange arrows) and a " />
</p>
...
Figure 15.1 illustrates this scheme. We start with a training set  $\lbrace x_{i} \rbrace$  of real 1D examples. A different batch of ten of these examples  $\lbrace x_{i} \rbrace_{i=1}^{10}$  is shown in each panel (cyan arrows). To create a batch of samples  $\lbrace x_{j}^{*} \rbrace$ , we use the simple generator:

$$
x_{j}^{*}=\mathrm{g}[z_{j},\theta]=z_{j}+\theta \quad (15.1)
$$

where latent variables $\lbrace z_{j} \rbrace$ are drawn from a standard normal distribution, and the parameter $\theta$ translates the generated samples along the x-axis (figure 15.1).

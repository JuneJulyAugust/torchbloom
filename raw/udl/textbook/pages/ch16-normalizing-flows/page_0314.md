...
$$
\begin{array}{rcl}h_{1}^{\prime}&=&\mathrm{g}\Big[h_{1},\phi\Big]\\h_{2}^{\prime}&=&\mathrm{g}\Big[h_{2},\phi[h_{1}]\Big]\\h_{3}^{\prime}&=&\mathrm{g}\Big[h_{3},\phi[h_{1:2}]\Big]\\h_{4}^{\prime}&=&\mathrm{g}\Big[h_{4},\phi[h_{1:3}]\Big].\end{array} \quad (16.16)
$$

 This must be inverted sequentially using a similar principle as for coupling flows: 

$$
\begin{array}{rcl}h_{1}&=&\mathrm{g}^{-1}\Big[h_{1}^{\prime},\phi\Big]\\h_{2}&=&\mathrm{g}^{-1}\Big[h_{2}^{\prime},\phi[h_{1}]\Big]\\h_{3}&=&\mathrm{g}^{-1}\Big[h_{3}^{\prime},\phi[h_{1:2}]\Big]\\h_{4}&=&\mathrm{g}^{-1}\Big[h_{4}^{\prime},\phi[h_{1:3}]\Big].\end{array} \quad (16.17)
$$
...
Residual flows take their inspiration from residual networks. They divide the input into two parts $h = [h_{1}^{T}, h_{2}^{T}]^{T}$ (as for coupling flows) and define the outputs as:

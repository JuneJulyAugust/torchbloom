...
$$
\mathrm{ELBO}[\boldsymbol{\theta},\phi]\quad\approx\quad\log\left[Pr(\mathbf{x}|\mathbf{z}^{*},\boldsymbol{\phi})\right]-\mathrm{D}_{KL}\Big[q(\mathbf{z}|\mathbf{x},\boldsymbol{\theta})\Big|\Big|Pr(\mathbf{z})\Big]. \quad (17.23)
$$
...
$$
\mathrm{D}_{KL}\left[q(\mathbf{z}|\mathbf{x},\boldsymbol{\theta})\Big|\big|Pr(\mathbf{z})\right]=\frac{1}{2}\bigg(\mathrm{Tr}[\boldsymbol{\Sigma}]+\boldsymbol{\mu}^{T}\boldsymbol{\mu}-D_{\mathbf{z}}-\log\left[\det[\boldsymbol{\Sigma}]\right]\bigg). \quad (17.24)
$$

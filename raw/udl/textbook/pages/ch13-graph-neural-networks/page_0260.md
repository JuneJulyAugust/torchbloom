...
figure_count: 0
---
Then the similarity $s\_{mn}$ of each transformed node embedding $\mathbf{h}\_{m}^{\prime}$ to the transformed node embedding $\mathbf{h}\_{n}^{\prime}$ is computed by concatenating the pairs, taking a dot product with a column vector $\phi\_{k}$ of learned parameters, and applying an activation function:

$$
s_{mn}
= \mathbf{a}\left[\boldsymbol{\phi}_{k}^{T}\begin{bmatrix}\mathbf{h}_{m}^{\prime}\\ \mathbf{h}_{n}^{\prime}\end{bmatrix}\right]
\qquad (13.23)
$$
...
$$
\mathbf{H}_{k+1}
= \mathbf{a}\left[\mathbf{H}_{k}^{\prime}\cdot\mathrm{Softmask}[\mathbf{S},\mathbf{A}+\mathbf{I}]\right]
\qquad (13.24)
$$

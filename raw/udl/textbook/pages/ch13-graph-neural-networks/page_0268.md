...
Problem 13.14 $^{*}$  Design a layer that updates a node embedding  $h_{m}$  based on its neighboring node embeddings  $\lbrace h_{m} \rbrace_{m\inne[n]}$  and neighboring edge embeddings  $\lbrace \mathbf{e}_{m} \rbrace_{m\innee[n]}$ . You should consider the possibility that the edge embeddings are not the same size as the node embeddings.

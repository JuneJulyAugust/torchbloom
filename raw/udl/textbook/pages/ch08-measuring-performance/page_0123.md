...
$$
\begin{aligned}
\mu[x]=\mathbb{E}_{y}[y[x]]=\int y[x]Pr(y|x)d y, \tag{8.1}
\end{aligned}
$$
...
$$
\begin{aligned}
\begin{align*}L[x]\quad&=\quad\left(\mathrm{f}[x,\phi]-y[x]\right)^{2}\\&=\quad\Big((\mathrm{f}[x,\phi]-\mu[x])+\big(\mu[x]-y[x]\big)\Big)^{2}\\&=\quad\big(\mathrm{f}[x,\phi]-\mu[x]\big)^{2}+2\big(\mathrm{f}[x,\phi]-\mu[x]\big)\big(\mu[x]-y[x])+\big(\mu[x]-y[x]\big)^{2},\end{align*} \tag{8.2}
\end{aligned}
$$
...
$$
\begin{aligned}
\begin{align*}\mathbb{E}_{y}\left[L[x]\right]&\quad=\quad\mathbb{E}_{y}\Big[\big(\mathrm{f}[x,\phi]-\mu[x]\big)^{2}+2\big(\mathrm{f}[x,\phi]-\mu[x]\big)\big(\mu[x]-y[x]\big)+\big(\mu[x]-y[x]\big)^{2}\Big]\\&=\quad\big(\mathrm{f}[x,\phi]-\mu[x]\big)^{2}+2\big(\mathrm{f}[x,\phi]-\mu[x]\big)\left(\mu[x]-\mathbb{E}_{y}\left[y[x]\right]\right)+\mathbb{E}_{y}\big[\big(\mu[x]-y[x]\big)^{2}\big]\\&=\quad\big(\mathrm{f}[x,\phi]-\mu[x]\big)^{2}+\sigma^{2},\end{align*} \tag{8.3}
\end{aligned}
$$

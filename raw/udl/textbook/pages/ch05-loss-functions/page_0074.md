...
$$
\begin{aligned}
\mathrm{s i g}[z]=\frac{1}{1+\exp[-z]}. \tag{5.32}
\end{aligned}
$$
...
**Problem 5.2** The loss L for binary classification for a single training pair $\lbrace x, y\rbrace$ is:
 $$
L = -(1 - y) \log\left[1 - \\mathrm{sig}[f[x, \phi]]\right] - y \log\left[\\mathrm{sign}[f[x, \phi]]\right],
$$
...
$$
\begin{aligned}
P r(y|\mu,\kappa)=\frac{\exp\left[\kappa\cos[y-\mu]\right]}{2\pi\cdot\mathrm{B e s s e l}_{0}[\kappa]}, \tag{5.34}
\end{aligned}
$$

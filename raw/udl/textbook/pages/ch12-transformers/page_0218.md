...
Each token in the vocabulary V is mapped to a unique word embedding, and the embeddings for the whole vocabulary are stored in a matrix  $\Omega_{e} \in R^{D \times |V|}$ . To accomplish this, the N input tokens are first encoded in the matrix  $T \in R^{|V| \times N}$ , where the  $n^{th}$  column corresponds to the  $n^{th}$  token and is a  $|V| \times 1$  one-hot vector (i.e., a vector where every entry is zero except for the entry corresponding to the token, which is set to one). The input embeddings are computed as  $X = \Omega_{e} T$ , and  $\Omega_{e}$  is learned like any other network parameter (figure 12.9). A typical embedding size D is 1024, and a typical total vocabulary size  $|V|$  is 30,000, so even before the main network, there are many parameters in  $\Omega_{e}$  to learn.

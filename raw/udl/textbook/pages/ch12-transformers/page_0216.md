...
---
<p align="center">
  <img src="../../figures/page_0216_fig_1.jpg" alt="Figure 12.7 Transformer layer. The input consists of a  $D \times N$  matrix containing the D-dimensional word embedding" />
</p>
...
$$
\begin{aligned}\mathbf{X}&\leftarrow\mathbf{X}+\mathrm{MhSa}[\mathbf{X}]\\ \mathbf{X}&\leftarrow\mathrm{LayerNorm}[\mathbf{X}]\\ \mathbf{x}_{n}&\leftarrow\mathbf{x}_{n}+\mathrm{mlp}[\mathbf{x}_{n}]\\\nabla n&\in\lbrace 1,\cdots,N\rbrace \end{aligned} \quad (12.13)
$$

where the column vectors  $x_{n}$  are separately taken from the full data matrix X. In a real network, the data passes through a series of these transformer layers.

# Writing Whole Numbers in Words

Source: https://www.mathacademy.com/topics/3885?courseId=75
Topic ID: 3885

## Prerequisites

- [Place Value With Numbers Up to Seven Digits](./3890-place-value-with-numbers-up-to-seven-digits.md)

## Lesson

### Introduction

In this lesson, we'll learn how to write numbers up to eight digits in words. We start by considering two-digit numbers.

Suppose we want to write the number $56$ in words. We start by writing down its place value chart.

Now, we write the number in words as follows:

- The number has $\color{blue}5$ tens, so we write "fifty," followed by a hyphen: $\qquad$ "*fifty-*"

- The number has $\color{red}6$ ones, so we add "six": $\qquad$ "*fifty-six*"

Therefore, the number $56$ written in words is *fifty-six.*

**Watch Out!** Don't forget to include the hyphen!

### Tables of Important Number Spellings

The table below gives the spellings of all whole numbers from $1$ to $20\mathbin{:}$

The table below gives the spellings of all multiples of $10$ up to $100\mathbin{:}$

### Example: Writing a Number With Two Digits in Words

#### Question

Write the number $81$ in words.

#### Explanation

To write a two-digit number (greater than $20$) in words, we

- write the tens part, followed by a hyphen

- then, write the ones part.

Let's write down the place values of $81\mathbin{:}$

Now, we write the number in words:

- The number has $\color{blue}8$ tens, so we write "eighty," followed by a hyphen: $\qquad$"**"

- The number has $\color{red}1$ ones, so we add "one": $\qquad$ "**"

Therefore, the number $81$ written in words is **.

### Three-Digit Numbers

Let's suppose we want to write the number $237$ in words.

We start by writing down its place value chart:

Now, we write the number in words:

- The number has $\color{blue}2$ hundreds, so we write "two" followed by "hundred": $\qquad$ "*two hundred*"

- The number has $\color{red}3$ tens and $\color{red}7$ ones, so we add "thirty-seven": $\qquad$ "*two hundred thirty-seven*"

Therefore, the number $237$ written in words is *two hundred thirty-seven*.

### Example: Writing a Number With Three Digits in Words

#### Question

Write the number in words.

#### Explanation

To write a three-digit number in words, we

- write the number of hundreds, followed by "hundred,"

- then, write the tens and ones part.

Let's write down the place values of

Now, we write the number in words:

- The number has hundreds, so we write "six" followed by "hundred": "**"

- The number has tens and ones, so we add "seven": "**"

Therefore, the number written in words is **.

### Four, Five, and Six-Digit Numbers

Suppose we want to write $8,236$ in words. Let's start by splitting the number into its so-called **periods** (parts of the number separated by commas):

$$

\boxed{8},\boxed{236}

$$

To express a four-digit number in words, we consider each period from left to right:

- $\boxed{8}$ has $8$ ones only, and this digit is in the thousands place. So, we write "eight" followed by "thousand,": $\qquad$ "*eight thousand,*"

- $\boxed{236}$ has $2$ hundreds, $3$ tens, and $6$ ones. So, we add "two hundred thirty-six": $\qquad$ "*eight thousand, two hundred thirty-six*"

Therefore, the number $8,236$ written in words is *eight thousand, two hundred thirty-six*.

We can use the same method to write down any number with four, five, or six digits in words. Let's see another example.

### Example: Writing a Number With Thousands in Words

#### Question

Write the number $32,042$ in words.

#### Explanation

Notice that the highest-value digit is in the ten-thousands place.

Let's start by splitting the number into its periods:

$$

\boxed{32},\boxed{042}

$$

Let's now look at each period:

- $\boxed{32}$ has $3$ tens and $2$ ones. So, we write "thirty-two" followed by "thousand,": $\qquad$ "**"

- $\boxed{042}$ has $0$ hundreds, $4$ tens, and $2$ ones. So, we add "forty-two": $\qquad$ "**"

Therefore, the number $32,042$ written in words is **.

### Seven and Eight-Digit Numbers

Let's write the following number in words:

$$

\boxed{2},\boxed{632},\boxed{125}

$$

To express a seven-digit number in words, we consider each period from left to right:

- $\boxed{2}$ has $2$ ones only, and this digit is in the millions place. So, we write "two" followed by "million,": $\qquad$ "*two million,*"

- $\boxed{632}$ has $6$ hundreds, $3$ tens and $2$ ones. So, we add "six hundred thirty-two," followed by "thousand,": $\qquad$ "*two million, six hundred thirty-two thousand,*"

- $\boxed{125}$ has $1$ hundred, $2$ tens and $5$ ones. So, we add "one hundred twenty-five": $\qquad$ "*two million, six hundred thirty-two thousand, one hundred twenty-five*"

Therefore, the number $2,632,125$ written in words is *two million, six hundred thirty-two thousand, one hundred twenty-five*.

We can use the same method to write down any number with seven, eight, or nine digits in words. Let's see another example.

### Example: Writing a Number With Millions in Words

#### Question

Write the number $15,081,012$ in words.

#### Explanation

Notice that the highest-value digit is in the ten-millions place.

Let's start by splitting the number into its periods:

$$

\boxed{15},\boxed{081},\boxed{012}

$$

Let's now look at each period:

- $\boxed{15}$ has $1$ ten and $5$ ones. So, we write "fifteen," followed by "million,": $\qquad$ "**"

- $\boxed{081}$ has $8$ tens and $1$ one. So, we add "eighty-one," followed by "thousand,": $\qquad$ "**"

- $\boxed{012}$ has $1$ ten and $2$ ones. So, we add "twelve": $\qquad$ "**"

Therefore, the number $15,081,012$ written in words is **.

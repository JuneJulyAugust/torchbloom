...
Given that $4a \equiv 4b \,(\textrm{mod}\,8),$ which of the following statements **** be true?
...
*provided that $k$ and $n$ are coprime*. In other words, we require $\textrm{gcd}(k,n)=1.$
...
- Consider the following true statement: Notice that the two numbers in our congruence have a common factor of ${\color{blue}2}{:}$ Since $\textrm{gcd}({\color{blue}2},5)=1,$ we can cancel out ${\color{blue}2}$ from both sides, keeping the modulus unchanged.

- Next, consider the following true statement: Notice that the two numbers in our congruence, *and the modulus*, have a common factor of ${\color{red}3}{:}$ Because $\textrm{gcd}({\color{red}3},6) = 3 \neq 1,$ we *cannot* cancel out ${\color{red}3}$ from both sides. If we do, we get the following *false* statement! In such cases, we should use the first division property to simplify the congruence.

### Example: Using the Second Division Property

#### Question

If $ka \equiv kb \: (\textrm{mod} \,7)$ is equivalent to $\%\qquad\Longrightarrow\qquad a \equiv b \:(\textrm{mod} \,7),$ which of the following could be the value of $k?$
...
Given that $15a \equiv 15b \,(\textrm{mod}\,20),$ which of the following statements **** be true?
...
- To prove the second division property, suppose that $\textrm{gcd}(k,n)=1$ and ${\color{blue}k}a\equiv {\color{blue}k}b \: (\textrm{mod} \,n).$ Then, according to the definition of congruent integers, we obtain Since $k$ and $n$ are coprime and $k(a - b)$ is divisible by $n,$ we have that $(a - b)$ must be divisible by $n.$ Now, using the definition again, we get

# Further Solving Multistep Word Problems Using the Four Operations

Source: https://www.mathacademy.com/topics/3978?courseId=75
Topic ID: 3978

## Prerequisites

- [Multi-Digit by One-Digit Multiplication Using Place Value](./2417-multi-digit-by-one-digit-multiplication-using-place-value.md)
- [Multi-Digit by One-Digit Division Using Partial Quotients](./2425-multi-digit-by-one-digit-division-using-partial-quotients.md)
- [Solving Multistep Word Problems Using Number Operations](./3904-solving-multistep-word-problems-using-number-operations.md)

## Lesson

### Introduction

In this lesson, we will learn how to use the four operations (addition, subtraction, multiplication, and division) to solve real-world multistep problems.

Let's take a look at the following situation:

*Chloe takes $18$ pieces of candy from a jar and another $15$ from a pack she bought. She plans to divide the candy equally between herself and two friends. How many pieces of candy will each person receive?*

We start by writing down the important information:

- Chloe took $18$ candy pieces from a jar.

- She has another $15$ pieces from a pack she bought.

- She wants to divide all the candy between $3$ people (herself, and two friends).

Let $N$ represent the number of pieces each person will receive. This is the number we want to find.

Now, we can deduce the following:

- Chloe took $18$ candy pieces from a jar, and she has $15$ more pieces from a pack she bought. Therefore, the total number of pieces equals

- The total number of candy pieces $({\color{blue}33})$ is then divided among $3$ people. Therefore, the total number of pieces each person will receive equals

We can use a box model to calculate $33\div 3{:}$

According to our model, $N = 33\div 3 = 10+1 = 11.$

Therefore, each person will receive $11$ candy pieces.

### Example: Solving Problems Involving Division Followed by Addition or Subtraction

#### Question

Thelma divided $225\, \text{g}$ of jelly beans equally among four of her friends and herself. Then, from her share, she gave $25\, \text{g}$ to her little brother. How many grams of jelly beans did she keep for herself?

#### Explanation

Let's start by writing down the important information:

- Thelma had $225\,\text{g}$ of jelly beans.

- The jelly beans were equally divided between $5$ people ($4$ of Thelma's friends, and herself).

- From her share, she gave $25\, \text{g}$ to her brother.

Let $B$ represent the number of grams of jelly beans Thelma kept for herself. This is the number we want to find.

Now, we can deduce the following:

- Thelma divided the $225\, \text{g}$ of jelly beans equally among $5$ people, including herself. Therefore, each of them received $225 \div 5 = {\color{blue}45}\, \text{g}$ of jelly beans.

- The weight of jelly beans that Thelma kept for herself, $B$, equals the amount we calculated above $({\color{blue}45}\, \text{g})$ minus the amount she gave to her brother $({\color{red}25}\, \text{g}).$ Therefore, the total number of grams of jelly beans Thelma kept for herself is

Calculating the final difference, we get

$$

B = 45 - 25 = 20.

$$

Therefore, Thelma kept only $20\, \text{g}$ of jelly beans for herself.

### Example: Solving Problems Involving Multiplication and Division

#### Question

A store clerk has to distribute $24$ packs of soda in a fridge, where each pack contains $6$ cans. If the cooler has $8$ shelves and she has to put the same number of cans on each shelf, how many cans of soda will she put on each shelf?

#### Explanation

Let's start by writing down the important information:

- There are $24$ packs of soda.

- Each pack contains $6$ soda cans.

- The fridge has $8$ shelves.

Let $S$ represent the total number of cans the clerk will put on each shelf. This is the number we want to find.

Now, we can deduce the following:

- There are $24$ packs of soda, and each pack contains $6$ cans. Therefore, the total number of soda cans is

- The total number of cans $({\color{blue}144})$ is divided equally among $8$ shelves. Therefore, the total number of cans of soda on each shelf is

Calculating the final quotient, we get $S = {\color{blue}144} \div 8 = 18{:}$

Therefore, the clerk will put $18$ cans of soda on each shelf.

### Example: Solving Multistep Word Problems With Remainders

#### Question

Britney is raising $742$ to buy a new laptop. She currently has $154,$ so to raise the rest of the money, she sells beaded bracelets for $5$ each. How many bracelets does Britney need to sell to raise the money she needs to buy the laptop?

#### Explanation

Let's start by writing down the important information:

- Britney wants to raise $742.$

- She currently has $154.$

- She sells bracelets for $5$ each.

Let $B$ represent the number of bracelets Britney needs to sell. This is the number we want to find.

Now, we can deduce the following:

- Britney wants to raise $742$, and she currently has $154.$ Therefore, the number of dollars she has left to raise is

- The number of bracelets Britney needs to sell, $B$, equals the number of dollars she has left to raise $(588)$ divided by the cost per bracelet $(5).$ Therefore, the number of bracelets Britney needs to sell is

Calculating the final quotient, we get $B = 588 \div 5 = {\color{blue}117}\,\text{R}\,{\color{red}3}{:}$

This means that, after selling $\color{blue}117$ bracelets, Britney still needs ${\color{red}3}.$

Therefore, Britney should sell ${\color{blue}117}+1=118$ bracelets.

...
Source: https://www.mathacademy.com/topics/1995?courseId=106
Topic ID: 1995

## Prerequisites

- [The Arc Length of a Parametric Curve](./997-the-arc-length-of-a-parametric-curve.md)
...
In the case of a parametric curve, the arc length element $\text{d}s$ is given by

$$


\text{d}s = \sqrt{\left(\dfrac{\text{d}x}{\text{d}t}\right)^2+\left(\dfrac{\text{d}y}{\text{d}t}\right)^2} \, \text{d}t.
...
S = 2\pi \int_{t_1}^{t_2} y(t) \sqrt{\left(\dfrac{\text{d}x}{\text{d}t}\right)^2+\left(\dfrac{\text{d}y}{\text{d}t}\right)^2}\,\text{d}t
...
![Instructional graphic](../../../lesson-assets/calculus-ii/topic-1995/a8d0e3edc30e00a8.png)
...
S = 2\pi \int_{t_1}^{t_2} x(t) \sqrt{\left(\dfrac{\text{d}x}{\text{d}t}\right)^2+\left(\dfrac{\text{d}y}{\text{d}t}\right)^2}\,\text{d}t.
...
We evaluate the last integral using the substitution $u = 1+t.$ Then, $1+3t=3u-2$ and $\text{d}u = \textrm d t.$ We also need to compute new limits, given below.

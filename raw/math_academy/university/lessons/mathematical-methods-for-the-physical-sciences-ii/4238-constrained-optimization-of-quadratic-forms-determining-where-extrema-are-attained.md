...
Source: https://www.mathacademy.com/topics/4238?courseId=155
Topic ID: 4238
...
- The maximum value of $Q(\mathbf{x}),$ when $\| \mathbf{x} \|=1,$ is equal to the largest eigenvalue $\lambda_{\text{max}}$ of $A,$ and the maximum is attained on the corresponding unit eigenvector $\mathbf{u}_{\text{max}}.$

- The minimum value of $Q(\mathbf{x}),$ when $\| \mathbf{x} \|=1,$ is equal to the smallest eigenvalue $\lambda_{\text{min}}$ of $A,$ and the minimum is attained on the corresponding unit eigenvector $\mathbf{u}_{\text{min}}.$
...
when $\| \mathbf{x} \| = 1$ is $6.$ Let's use this to find a unit vector $\mathbf{u}_{\text{max}}$ where this maximum value is attained.
...
Next, we find the unit eigenvector that corresponds to the eigenvalue $\lambda_{\text{max}}=6{:}$

$$


\begin{aligned}𝐴−6𝐼 & =[\begin{matrix}1 & −5 \\ −5 & 1\end{matrix}]−6[\begin{matrix}1 & 0 \\ 0 & 1\end{matrix}]=[\begin{matrix}−5 & −5 \\ −5 & −5\end{matrix}]\end{aligned}
...
Now, we need the unit eigenvector that corresponds to the eigenvalue $\lambda_{\text{min}}=0{:}$

$$


\begin{aligned}𝐴−0𝐼 & =[\begin{matrix}9 & −12 \\ −12 & 16\end{matrix}]−[\begin{matrix}0 & 0 \\ 0 & 0\end{matrix}]=[\begin{matrix}9 & −12 \\ −12 & 16\end{matrix}]\end{aligned}
...
Now, we need the unit eigenvector that corresponds to the eigenvalue $\lambda_{\text{max}}=6{:}$

$$


\begin{aligned}𝐴−6𝐼 & =\begin{matrix}−1 & 2 & 0 \\ 2 & 2 & 0 \\ 0 & 0 & 6\end{matrix}−6\begin{matrix}1 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1\end{matrix}=\begin{matrix}−7 & 2 & 0 \\ 2 & −4 & 0 \\ 0 & 0 & 0\end{matrix}\end{aligned}
...
\begin{aligned}𝐵 & =𝑃^{𝑇}\,𝐴𝑃 \\ & =𝑃𝐴𝑃 \\ & =[\begin{matrix}1 & 0 \\ 0 & 2\end{matrix}][\begin{matrix}1 & −1 \\ −1 & 1\end{matrix}][\begin{matrix}1 & 0 \\ 0 & 2\end{matrix}] \\ & =[\begin{matrix}1 & −1 \\ −2 & 2\end{matrix}][\begin{matrix}1 & 0 \\ 0 & 2\end{matrix}] \\ & =[\begin{matrix}1 & −2 \\ −2 & 4\end{matrix}].\end{aligned}
...
Next, we find the unit eigenvector that corresponds to the eigenvalue $\lambda_{\text{max}}=5{:}$

$$


\begin{aligned}𝐵−5𝐼 & =[\begin{matrix}1 & −2 \\ −2 & 4\end{matrix}]−5[\begin{matrix}1 & 0 \\ 0 & 1\end{matrix}]=[\begin{matrix}−4 & −2 \\ −2 & −1\end{matrix}]\end{aligned}
...
\begin{aligned}𝐱_{max} & =𝑃𝐮_{max} \\ & =[\begin{matrix}1 & 0 \\ 0 & 2\end{matrix}]⋅\frac{1}{\sqrt{5}}[\begin{matrix}−1 \\ 2\end{matrix}] \\ & =\frac{1}{\sqrt{5}}[\begin{matrix}−1 \\ 4\end{matrix}]\end{aligned}
...
\begin{aligned}𝐵 & =𝑃^{𝑇}\,𝐴𝑃 \\ & =𝑃𝐴𝑃 \\ & =[\begin{matrix}1 & 0 \\ 0 & 4\end{matrix}][\begin{matrix}16 & 8 \\ 8 & 1\end{matrix}][\begin{matrix}1 & 0 \\ 0 & 4\end{matrix}] \\ & =[\begin{matrix}16 & 8 \\ 32 & 4\end{matrix}][\begin{matrix}1 & 0 \\ 0 & 4\end{matrix}] \\ & =[\begin{matrix}16 & 32 \\ 32 & 16\end{matrix}].\end{aligned}
...
\begin{aligned}|𝐵−𝜆𝐼| & =0 \\ \begin{matrix}16−𝜆 & 32 \\ 32 & 16−𝜆\end{matrix} & =0 \\ (16−𝜆)(16−𝜆)−1\,024 & =0 \\ 𝜆^{2}−32𝜆−768 & =0 \\ (𝜆−48)(𝜆+16) & =0 \\ 𝜆 & =48,\,−16\end{aligned}
...
\min \{ Q'(\mathbf{y}) \: | \: \| \mathbf{y} \|=1 \} = \lambda_{\text{min}} = -16.


$$

Next, we need the unit eigenvector that corresponds to the eigenvalue $\lambda_{\text{min}}=-16{:}$

$$


\begin{aligned}𝐵−(−16𝐼) & =[\begin{matrix}16 & 32 \\ 32 & 16\end{matrix}]+16[\begin{matrix}1 & 0 \\ 0 & 1\end{matrix}]=[\begin{matrix}32 & 32 \\ 32 & 32\end{matrix}]\end{aligned}
...
\begin{aligned}𝐱_{min} & =𝑃𝐮_{min} \\ & =[\begin{matrix}1 & 0 \\ 0 & 4\end{matrix}]⋅\frac{1}{\sqrt{2}}[\begin{matrix}−1 \\ 1\end{matrix}] \\ & =\frac{1}{\sqrt{2}}[\begin{matrix}−1 \\ 4\end{matrix}]\end{aligned}
...
\max \{ Q(\mathbf{x}) \: | \: \| \mathbf{x} \| = 1 \} = \lambda_{\text{max}}


$$

*The maximum is attained on the corresponding unit eigenvector $\mathbf{u}_{\text{max}}.$*
...
- The diagonal entries of $D$ are the eigenvalues of $A$. Without loss of generality, we can assume that the eigenvalues are $\lambda_1 \geq \lambda_2 \geq \ldots \geq \lambda_n,$ where $\lambda_{\text{max}} = \lambda_1.$

- The columns $\mathbf{u}_1,\mathbf{u}_2, \ldots, \mathbf{u}_n$ of $P$ form an orthonormal set of unit eigenvectors of $A.$ So, $\mathbf{u}_{\text{max}} = \mathbf{u}_1.$
...
M = \max \{ Q(\mathbf{x}) \: | \: \| \mathbf{x} \| = 1 \} = \max \{ Q'(\mathbf{y}) \: | \: \| \mathbf{y} \| = 1 \} \le \lambda_{\text{max}}.


$$

On the other hand, if we take $[\begin{aligned}1 & 0 & … & 0\end{aligned}]$ then $Q(P\mathbf{e}_1) = Q'(\mathbf{e}_1) = \lambda_{\text{max}}.$ So, we conclude that $M = \lambda_{\text{max}}.$

Finally, since $\mathbf{u}_1 = P\mathbf{e}_1$ we have that $Q(\mathbf u_1) = \mathbf e_1^T D \mathbf e_1 = \lambda_{\text{max}}.$ Therefore, the maximum value is attained at $\mathbf u_{\text{max}}.$

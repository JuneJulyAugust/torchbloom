...
\text{Var}[X\,|\,Y] = \text{E}\big[(X - \mu_{X|y} \big)^2 \,|\, Y=y].
...
\displaystyle \text{E}\big[X^2 \,| \, Y=y \big] = \int_{-\infty}^{\infty} x^2 \, f_{X|Y}(x \,|\, y) \: \text{d}x,


$$

and $f_{X|Y}(x \,|\, y)$ is the conditional probability density function of $X$ given $Y=y.$ Note that these formulas are analogous to those for $\text{Var}[X]$ for continuous $X.$
...
- $\mu_{Y|x} = \text{E}\big[Y \,| \, X=x \big]$ is the conditional expected value of $Y$ given $X=x,$

- $\displaystyle \text{E}\big[Y^2 \,| \, X=x \big] = \int_{-\infty}^{\infty} y^2 \, f_{Y|X}(y \,|\, x) \: \text{d}y,$ and
...
What is the conditional variance $\text{Var}\big[Y \,|\, X=1\big]?$
...
\text{E}\big[Y^2 \,| \, X=x \big] = \int_{-\infty}^{\infty} y^2 \, f_{Y|X}(y \,|\, x) \: \text{d}y.
...
Let $X$ and $Y$ be continuous random variables. Find the conditional variance $\text{Var}\big[X \,|\, Y=-2\big]$ given that the conditional probability density function $f_{X|Y}(x \,|\, -2)$ is shown above and the corresponding expected value is $\mu_{X|-2}=\dfrac{3}{4}.$
...
\text{E}\big[X^2 \,| \, Y=y \big] = \int_{-\infty}^{\infty} x^2 \, f_{X|Y}(x \,|\, y) \: \text{d}x.
...
Let $X$ and $Y$ be continuous random variables. Find the conditional variance $\text{Var}\big[Y \,|\, X=3\big]$ given that the conditional probability density function $f_{Y|X}(y \,|\, 3)$ is shown above and the corresponding expected value is $\mu_{Y|3}=\dfrac{3}{2}.$
...
\begin{aligned}𝑓(𝑥,𝑦)=\begin{matrix}2𝑦ln⁡(𝑥), & 1≤𝑥≤𝑒,\,0≤𝑦≤1, \\ 0, & otherwise\end{matrix}\,𝑓_{𝑋}(𝑥)=\begin{matrix}ln⁡(𝑥), & 1≤𝑥≤𝑒, \\ 0, & otherwise\end{matrix}\end{aligned}


$$

Let $X$ and $Y$ be continuous random variables. Their joint probability density function and the marginal probability density function of $X$ are shown above. Find the conditional variance $\text{Var}\big[Y \,\big|\, X=1 \big]$ given that the corresponding expected value is $\mu_{Y|1}=\dfrac{2}{3}.$

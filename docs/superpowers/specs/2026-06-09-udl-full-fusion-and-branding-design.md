...
Display equations use `$$` delimiters so they render in GitHub Markdown and local previews. Do not publish pages with `\[` and `\]` display delimiters. Do not publish `\tag{...}` equation numbering; GitHub's sanitizer can reject or vertically stack tagged equations. Put visible equation numbers inside the display as plain math text such as `\quad (15.1)`. Inline math uses `$...$`; footnotes use Markdown footnote syntax such as `[^1]`, not `$^1$` markers.
